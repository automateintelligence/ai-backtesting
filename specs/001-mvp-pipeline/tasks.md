...
## Phase 7a: User Story 6a - Distribution Model Validation and Selection (Priority: P2)

**Goal**: Automated distribution audit for ticker symbols to fit multiple return distribution models (Laplace, Student-t, GARCH-t), validate through comprehensive statistical tests, and recommend best-fit model based on empirical evidence of tail behavior, volatility clustering, and simulation realism

**Independent Test**: For SPY with 3 years of daily data, run distribution audit CLI. Verify system fits all three models, computes AIC/BIC, performs VaR backtesting with Kupiec/Christoffersen tests, validates simulation realism, and outputs ranked model comparison report with clear recommendation per spec.md US6a acceptance scenarios

### Implementation for User Story 6a

#### Foundation and Stub Integration (US6a)

- [X] T137 [US6a] Review and adapt existing distribution_audit.py stub in qse/distributions/distribution_audit.py - NOTE: This file contains sophisticated model comparison logic that MUST be adapted to our framework, not rewritten. Extract existing fit testing methodology, statistical test implementations, and model comparison algorithms. Map existing functions to US6a acceptance scenarios (AS1-AS12) and plan integration with new modules below per spec.md US6a requirements

#### Preprocessing and Model Fitting (US6a AS1)

- [X] T138 [P] [US6a] Implement LaplaceFitter class in qse/distributions/fitters/laplace_fitter.py with MLE estimation for location μ and scale b parameters per spec.md US6a AS1 (include convergence tracking, use scipy.stats.laplace.fit(), integrate with distribution_audit.py stub logic from T137)
- [X] T139 [P] [US6a] Implement StudentTFitter class in qse/distributions/fitters/student_t_fitter.py with MLE estimation for μ, σ, ν parameters per spec.md US6a AS1 (track iterations, convergence status, leverage T137 stub patterns)
- [X] T140 [US6a] Implement GARCHTFitter class in qse/distributions/fitters/garch_t_fitter.py with MLE for ω, α, β, ν and standardized residuals z_t = r_t / σ̂_t per spec.md US6a AS1 (use arch library, extract residuals, integrate T137 methodology)
- [X] T141 [P] [US6a] Create stationarity validation module in qse/distributions/validation/stationarity.py checking minimum sample sizes (Laplace ≥60, Student-t ≥60, GARCH-t ≥252) per FR-032/spec.md US6a AS1
- [X] T142 [US6a] Implement FitResult dataclass in qse/distributions/models.py capturing fitted parameters, convergence status, iterations, warnings per spec.md US6a AS1 (include model_type, params dict, converged bool, n_iterations, warnings list)

#### Goodness-of-Fit Metrics (US6a AS2)

- [X] T143 [P] [US6a] Implement AIC/BIC computation in qse/distributions/metrics/information_criteria.py calculating AIC = 2k - 2ℓ and BIC = k×log(n) - 2ℓ per spec.md US6a AS2 (check T137 stub for existing implementation)
- [X] T144 [P] [US6a] Create model ranking logic in qse/distributions/metrics/model_ranking.py sorting models by AIC/BIC (lower is better) with interpretation guidance per spec.md US6a AS2
...
- [X] T146 [P] [US6a] Implement QQ plot generation in qse/distributions/diagnostics/qq_plots.py comparing empirical vs model quantiles focusing on ±2σ tails per spec.md US6a AS3 (use matplotlib/seaborn, leverage T137 stub visualizations)
- [X] T147 [P] [US6a] Create tail error metric calculator in qse/distributions/diagnostics/tail_metrics.py computing tail_error_99 = |VaR_model(0.99) - VaR_emp(0.99)| / |VaR_emp(0.99)| at 95%, 99%, 99.5% quantiles per spec.md US6a AS3
- [X] T148 [P] [US6a] Implement excess kurtosis validation in qse/distributions/diagnostics/kurtosis.py comparing fitted vs empirical kurtosis with ≥1.0 requirement per spec.md US6a AS3/FR-037
...
- [X] T156 [US6a] Implement MC path generator for validation in qse/distributions/validation/mc_path_generator.py generating 10,000 paths × 252 steps per model per spec.md US6a AS5
- [X] T157 [P] [US6a] Create annualized volatility calculator in qse/distributions/validation/volatility_calc.py computing σ_annual = σ_daily × √252 per spec.md US6a AS5
- [X] T158 [P] [US6a] Implement autocorrelation calculator in qse/distributions/validation/clustering_calc.py computing corr(r²_t, r²_{t-1}) for volatility clustering per spec.md US6a AS5
- [X] T159 [P] [US6a] Create maximum drawdown calculator in qse/distributions/validation/drawdown_calc.py computing max percentage decline from peak per spec.md US6a AS5
- [X] T160 [P] [US6a] Implement extreme move counter in qse/distributions/validation/extreme_moves.py counting |r_t| > 3% and |r_t| > 5% per spec.md US6a AS5
- [X] T161 [US6a] Create historical metrics calculator in qse/distributions/validation/historical_metrics.py computing same statistics on 252-day rolling windows per spec.md US6a AS5
- [X] T162 [US6a] Implement distributional comparator in qse/distributions/validation/distribution_compare.py comparing simulated vs historical metric distributions per spec.md US6a AS5
- [X] T163 [US6a] Create simulation realism report generator in qse/distributions/validation/realism_report.py with histograms, systematic bias detection per spec.md US6a AS5

#### Model Selection and Scoring (US6a AS6)

- [X] T164 [US6a] Implement AIC normalization in qse/distributions/selection/normalize.py computing AIC_norm = (AIC - AIC_min) / (AIC_max - AIC_min) per spec.md US6a AS6
- [X] T165 [US6a] Create composite scoring function in qse/distributions/selection/scorer.py implementing score = w₁×(-AIC_norm) + w₂×(-tail_error_99) + w₃×(-VaR_backtest_penalty) + w₄×(-vol_cluster_error) with default weights (0.2, 0.4, 0.3, 0.1) per spec.md US6a AS6
- [X] T166 [US6a] Implement constraint validator in qse/distributions/selection/constraints.py enforcing excess kurtosis ≥1.0 and VaR backtest pass (p ≥ 0.01 on at least one test) per spec.md US6a AS6
- [X] T167 [US6a] Create model selector in qse/distributions/selection/model_selector.py choosing highest-scoring model meeting constraints per spec.md US6a AS6
- [X] T168 [US6a] Implement selection report generator in qse/distributions/selection/selection_report.py documenting rationale with evidence and warnings per spec.md US6a AS6

#### Caching and Reproducibility (US6a AS7-8)

- [X] T169 [P] [US6a] Implement cache manager in qse/distributions/cache/cache_manager.py with cache key (symbol, lookback_days, end_date, data_source) and 30-day TTL per spec.md US6a AS7
- [X] T170 [P] [US6a] Create audit result serializer in qse/distributions/cache/serializer.py storing fitted parameters, validation metrics, selection decision as JSON per spec.md US6a AS7
...
- [X] T173 [US6a] Create model loader in qse/distributions/integration/model_loader.py auto-loading cached validated models for US1/US6 runs per spec.md US6a AS9
- [X] T174 [US6a] Implement model metadata logger in qse/distributions/integration/metadata_logger.py recording model type, fit date, validation scores in run_meta.json per spec.md US6a AS9/FR-034
- [X] T175 [US6a] Add cache age warning in qse/distributions/integration/cache_checker.py suggesting re-audit when cache > 30 days old per spec.md US6a AS9
- [X] T176 [US6a] Implement fallback handler in qse/distributions/integration/fallback.py emitting warning and using Laplace default when no audit exists, marking run_meta with model_validated: false per spec.md US6a AS10

#### CLI Command (US6a)

- [X] T177 [US6a] Create audit-distributions CLI command in qse/cli/commands/audit_distributions.py accepting --symbol, --lookback-days, --end-date, --force-refit flags per spec.md US6a Independent Test
- [X] T178 [US6a] Wire audit CLI to DistributionAuditor orchestrator in qse/distributions/distribution_audit.py coordinating all validation steps per spec.md US6a AS1-AS12
- [X] T179 [US6a] Add audit results output formatter in qse/cli/formatters/audit_formatter.py displaying ranked models, scores, recommendation per spec.md US6a AS6

#### Error Handling (US6a AS11-12)

- [X] T180 [P] [US6a] Implement insufficient data handler in qse/distributions/errors/data_errors.py skipping models when sample size < minimum (GARCH-t ≥252) with warning per spec.md US6a AS11
- [X] T181 [P] [US6a] Create convergence failure handler in qse/distributions/errors/convergence_errors.py logging diagnostics, marking model "FAILED", continuing with others per spec.md US6a AS12
- [X] T182 [US6a] Implement audit failure logic in qse/distributions/distribution_audit.py failing entire audit only if all three models fail to converge per spec.md US6a AS12
